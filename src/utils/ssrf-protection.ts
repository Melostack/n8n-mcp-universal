...
export class SSRFProtection {
   ...
   */
  /**
   * Generates a custom DNS lookup function for Axios to prevent DNS rebinding attacks.
   * This pins the IP address to the one validated by validateWebhookUrl.
   *
   * @param resolvedIP The validated IP address
   * @param family The IP family (4 or 6)
   * @returns A lookup function compatible with Axios/Node.js http module
   */
  static getAxiosLookup(resolvedIP: string, family: number) {
    return (
      hostname: string,
      options: any,
      callback: (err: NodeJS.ErrnoException | null, address: string, family: 4 | 6) => void
    ) => {
      // Regardless of what hostname Axios asks for, return our pre-validated IP
      logger.debug('DNS pinning applied', { hostname, pinnedIP: resolvedIP });
      callback(null, resolvedIP, family as 4 | 6);
    };
  }
  ...
  static async validateWebhookUrl(urlString: string): Promise<{
    ...
    try {
      ...
      if (mode === 'permissive') {
        ...
        return { valid: true, resolvedIP, family: resolvedFamily };
      }
