...
export class SSRFProtection {
   ...
   */
  static validateUrlSync(urlString: string, modeOverride?: SecurityMode): {
    valid: boolean;
    reason?: string
  } {
    try {
      const url = new URL(urlString);
      // Default to permissive for API URL unless N8N_API_SECURITY_MODE is set
      const defaultMode = (process.env.N8N_API_SECURITY_MODE || 'permissive') as SecurityMode;
      const mode: SecurityMode = modeOverride || defaultMode;
